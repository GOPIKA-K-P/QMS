...
from utils.commonlib import response_json
...
class NoSqlBuilder:
    ...
    def get(self, query_param, projection=None):
        """
        NosqlDB get method
    ...
    def search(
        ...
        try:
            ...
            condition["ScanIndexForward"] = order_by
            if projection:
                condition["ProjectionExpression"] = ", ".join(projection)
            ...
            logging.info(f"exception:{e.response}")
            return response_json(False, str(e))
    ...
    def _extracted_response_validation(self, result):
        ...
        result = {"records": result['Items'], "count": result['Count']}
